...
function createConsortium() {
  ...
  set -x
  configtxgen -profile FoodChainOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
COMPOSE_FILE_BASE=docker/docker-compose-namek-net.yaml
